...
@section('pageTitle' , isset($task) ? $task->title : 'Create objective')
@section('content')
<!-------------->
        ...
        <div class="card mb-5 mb-xl-10">
            ...
            <div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse" data-bs-target="#kt_account_profile_details" aria-expanded="true" aria-controls="kt_account_profile_details">
                ...
                <div class="card-title m-0">
                    <h3 class="fw-bolder m-0">{{ isset($task) ? $task->title : 'Create objective' }}</h3>
                </div>
                <!--end::Card title-->
            </div>
            <!--begin::Card header-->
            ...
            <div id="kt_account_settings_profile_details" class="collapse show">
                ...
                <form id="kt_ecommerce_add_product_form" 
                    data-kt-redirect="{{  isset($task) ? route('objective.edit',$task->id) : route('objective.create') }}" 
                action="{{ isset($task) ? route('objective.update', $task->id) : route('objective.store') }}"
                    ...
                    <div class="card-body border-top p-9">
                        ...
                        <div class="row ">
                            ...
                            <div class="com-md-12 mt-5">
                                    ...
                                    <select required  name="task_id" required aria-label="Select a Country" data-control="select2" data-placeholder="select objective" 
                                    class="form-select form-select-solid form-select-lg fw-bold select2-hidden-accessible" data-select2-id="select2-data-10-05ls" tabindex="-1" aria-hidden="true">
                                        <option value="" data-select2-id="select2-data-12-wa4o">select objective</option>
                                        @foreach ($tasks as $selectTask)
                                        <option value="{{$selectTask->id}}" 
                                            @if(isset($task)) 
                                                {{$task->task_id  == $selectTask->id ? "selected" : ""}}
                                            @else 
                                                {{old('task_id')  == $selectTask->id ? "selected" : ""}}
                                            @endif 
                                            >{{ $selectTask->title }}</option>
                                        @endforeach
                                    </select>
                                    <div class="fv-plugins-message-container invalid-feedback"></div>
                            </div>
